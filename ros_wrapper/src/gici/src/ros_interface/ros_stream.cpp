  ...
  Streaming(), nh_(nh), frame_id_("World"), valid_(false)
{
  ...
  // --sbs
  else if (data_format == "odometer") {
    data_format_ = RosDataFormat::Odometer;
    if (io_type_ == StreamIOType::Input) {
      subscribers_.push_back(nh_.subscribe<nav_msgs::Odometry>(
        topic_name_, queue_size_, boost::bind(&RosStream::odomCallback, this, _1)));
    }
    else {
  ...
  else if (data_format == "diagnostic") {
    data_format_ = RosDataFormat::Diagnostic;
    if (io_type_ == StreamIOType::Input) {
      LOG(ERROR) << "Setting diagnostic topic as input is disabled!";
      return;
    }
    else {
      publishers_.push_back(nh_.advertise<diagnostic_msgs::DiagnosticStatus>(topic_name_, queue_size_));
    }
  }
...
void RosStream::solutionOutputCallback(std::string tag, Solution& solution)
{
  ...
  // --sbs
  else if (data_format_ == RosDataFormat::Diagnostic) {
    publishDiagnostic(publishers_[0], solution.num_satellites, solution.PDOP,
      ros::Time(solution.timestamp));
  }
}
...
void RosStream::compressedImageCallback(const sensor_msgs::CompressedImageConstPtr& msg)
{
  // Note that we set MONO8 as default, if the input image is colored image, 
  // you should add a convertion here
  cv_bridge::CvImageConstPtr ptr = 
...
void RosStream::odomCallback(const nav_msgs::OdometryConstPtr& msg)
{
  // control the frequency of odom data under 10 Hz--sbs
  double timestamp = msg->header.stamp.toSec();
  if( last_odom_time_ > 0 && timestamp - last_odom_time_ < 0.1) {
    return;
  }
  ...
  // Convert odom data
  std::shared_ptr<DataCluster> data_cluster = 
    std::make_shared<DataCluster>(FormatorType::Odometer);

  data_cluster->odometer->time = msg->header.stamp.toSec();
  data_cluster->odometer->forward_velocity = msg->twist.twist.linear.x;
  data_cluster->odometer->bearing_angular_velocity = msg->twist.twist.angular.z;
  
  // Call Odom processor
  for (auto it_odom_callback : data_callbacks_) {
    it_odom_callback(tag_, data_cluster);
  }
