...
#include <diagnostic_msgs/DiagnosticStatus.h> //--sbs
#include <diagnostic_msgs/KeyValue.h> //--sbs
...
// Publish Diagnostic
void publishDiagnostic(ros::Publisher& pub, const double num_satellites, const double PDOP, const ros::Time time)
{
  // Publish Diagnostic
  diagnostic_msgs::DiagnosticStatus diagnostic_msg;

  diagnostic_msg.name = "M1 naviagation status";
  diagnostic_msg.hardware_id = "M1";
  diagnostic_msg.level = 3;
  diagnostic_msg.message = "values: {key = satellite, value = PDOP}";
  diagnostic_msgs::KeyValue key_value;
  key_value.key = std::to_string(num_satellites);
  key_value.value = std::to_string(PDOP);
  diagnostic_msg.values.push_back(key_value);

  pub.publish(diagnostic_msg);
}
