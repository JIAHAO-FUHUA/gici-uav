...
void RosNodeHandle::rebindAllStreamerToEstimator(const NodeOptionHandlePtr& nodes)
{
  ...
  for (size_t i = 0; i < estimatings_.size(); i++) {
    ...
    // distinguish sensors
    std::vector<std::string> gnss_tags, imu_tags, image_tags, solution_tags, odom_tags;
    std::vector<std::vector<std::string>> gnss_roles, imu_roles, image_roles, solution_roles, odom_roles;
    std::vector<std::shared_ptr<Streaming>> 
      gnss_streamings, imu_streamings, image_streamings, solution_streamings, odom_streamings;
    for (size_t i = 0; i < input_tags.size(); i++) {
      ...
      //--sbs
      else if (option_tools::sensorType(roles[0]) == SensorType::Odometer) {
        odom_tags.push_back(input_tag);
        odom_roles.push_back(roles);
        std::shared_ptr<Streaming> stream = is_ros ? 
          getRosStreamFromTag(input_tag) : getStreamFromFormatorTag(input_tag);
        if (!is_ros) {
          bool found = false;
          for (auto streaming : odom_streamings) {
            if (stream->getTag() == streaming->getTag()) {
              found = true; break;
            }
          }
          if (!found) odom_streamings.push_back(stream);
        }
        else odom_streamings.push_back(stream);
      }
    }
    ...
    data_integrations.push_back(std::make_shared<OdomDataIntegration>(
      estimating, odom_streamings, odom_tags, odom_roles));
