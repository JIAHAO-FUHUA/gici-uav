  ...
  compute_covariance_(true)
{
  ...
  if (!option_tools::safeGet(node, "is_post_process", &is_post_process_)) {
    is_post_process_ = false;
  }
  if (!option_tools::safeGet(node, "use_precise_product", &use_precise_product_)) {
    use_precise_product_ = false;
  } 
  if (!option_tools::safeGet(node, "use_base_position_cfile", &use_base_position_cfile_)) {
    use_base_position_cfile_ = false;
  }

  std::vector<double> base_position;
  if (option_tools::safeGet(node, "base_position_cfile", 
      &base_position) && 
      base_position.size() == 3) {
    for (size_t i = 0; i < 3; i++) {
      base_position_cfile_[i] = base_position[i];
    }
  }
  else {
    LOG(INFO) << "Unable to load base_position_cfile. Using default instead.";
    base_position_cfile_ = Eigen::Vector3d::Zero();
