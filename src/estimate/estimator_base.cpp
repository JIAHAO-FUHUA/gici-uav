...
{
  // check if we have marginalization error  --sbs
  if(marginalization_parameter_ids_.size() == 0)
  {
    return false;
  }
  ...
  if(marginalization_error_->num_residuals()==0)
  {
    ...
    return false; // nothing to marginalize --sbs
  }
...
Eigen::MatrixXd EstimatorBase::computeAndGetCovarianceAR
      (const State& state,
      const AmbiguityState& ambiguity_state)
{
  BackendId id = state.id_in_graph;
  Eigen::MatrixXd full_covariance;
  const size_t amb_size = ambiguity_state.ids.size();
  size_t full_size = 3 + amb_size;
  full_covariance.resize(full_size, full_size);
  ...
  if (id.type() == IdType::gPosition)
  {
    ...
    // ambiguity ids
    for (auto amb_id : ambiguity_state.ids) {
      parameter_block_ids.push_back(amb_id.asInteger());
    }
    // check if we have velocity estimate
    ...
    if (!graph_->computeCovariance(parameter_block_ids, covariance)) {
      LOG(ERROR)<< "compute position + ambiguity covariance failed!";
  ...
  else
  {
    ...
    full_covariance.block(0,0,3,3)          =      covariance.block(0,  0,  3,  3);
    full_covariance.block(0,3,3,amb_size)   =      covariance.block(0,  16, 3,  amb_size);
    full_covariance.block(3,0,amb_size,3)   =      covariance.block(16, 0,  amb_size,3);
    full_covariance.block(3,3,amb_size,amb_size) = covariance.block(16,16,amb_size,amb_size);
    // convert to minimal
