...
#include "gici/imu/odom_estimator_base.h"  //--sbs
// #include "gici/fusion/ppprtk_imu_tc_estimator.h"
  ...
  (const std::string& in, FormatorType& out)
{
  ...
  MAP_IN_OUT("rnx-o-file", FormatorType::RnxOFile);  //--sbs
  MAP_IN_OUT("rnx-n-file", FormatorType::RnxNFile);  //--sbs
  MAP_IN_OUT("osb-file", FormatorType::OsbFile);
  MAP_IN_OUT("sp3-file", FormatorType::Sp3File);
  MAP_IN_OUT("clk-file", FormatorType::ClkFile);  
  MAP_IN_OUT("ion-file", FormatorType::IonFile);
  MAP_IN_OUT("ztd-file", FormatorType::ZtdFile);
  MAP_IN_OUT("imu-imr", FormatorType::IMUImr);
  MAP_IN_OUT("image-file", FormatorType::ImageFile);
  LOG_INVALId;
}
...
// related to estimator.fmt_xxx_roles --sbs
template <>
  ...
  (const std::string& in, GnssRole& out)
{
  ...
  MAP_IN_OUT("ionosphere", GnssRole::Ionosphere);  //--sbs
  MAP_IN_OUT("troposphere", GnssRole::Troposphere);
  MAP_IN_OUT("sp3_ephemeris", GnssRole::Sp3Ephemeris);
  MAP_IN_OUT("satellite_clock", GnssRole::SatelliteClock);
  MAP_IN_OUT("observation_bias", GnssRole::ObservationBias);
  LOG_INVALId;
}
...
template <>
void convert<std::string, OdomRole>
  (const std::string& in, OdomRole& out)
{
  MAP_IN_OUT("skid", OdomRole::Skid);
  LOG_INVALId;
}
  ...
  (const std::string& in, EstimatorType& out)
{
  ...
  MAP_IN_OUT("ppprtk_imu_tc", EstimatorType::PpprtkImuTc);  //--sbs
  LOG_INVALId;
}
...
// Get sensor type from options, related to estimator.fmt_xxx_roles --sbs
SensorType sensorType(std::string in)
{
  ...
  MAP_IN_RET("ionosphere", SensorType::GNSS);  //--sbs
  MAP_IN_RET("troposphere",       SensorType::GNSS);
  MAP_IN_RET("sp3_ephemeris",     SensorType::GNSS);
  MAP_IN_RET("satellite_clock",   SensorType::GNSS);
  MAP_IN_RET("observation_bias",  SensorType::GNSS);
  ...
  MAP_IN_RET("skid",  SensorType::Odometer);  //--sbs
  ...
  MAP_IN_RET("imu-imr", SensorType::IMU);  //--sbs
  MAP_IN_RET("image-file", SensorType::Camera);
  MAP_IN_RET("option", SensorType::Option);
    ...
    YAML::Node& node, GnssCommonOptions& options)
{
  ...
  LOAD_COMMON(psr_dop_thres  );
  LOAD_COMMON(delta_dop_thres);
  LOAD_COMMON(phr_dop_thres  );
  LOAD_COMMON(psr_phr_thres  );
    ...
    YAML::Node& node, AmbiguityResolutionOptions& options)
{
  ...
  LOAD_COMMON(min_num_satellite_pairs_fixation);
  LOAD_COMMON(ratio);
    ...
    YAML::Node& node, GnssEstimatorBaseOptions& options)
{
  ...
  LOAD_COMMON(max_pesudorange_size);
  LOAD_COMMON(max_satellite_size);
...
template <>
void loadOptions<OdomEstimatorBaseOptions>(
    YAML::Node& node, OdomEstimatorBaseOptions& options)
{
  LOAD_COMMON(use_forward_velocity);  // odometer --sbs
  LOAD_COMMON(forward_velocity_std);  // odometer --sbs
  LOAD_COMMON(forward_velocity_rw);  // odometer --sbs
  LOAD_COMMON(bearing_angular_velocity_std);  // odometer --sbs
  LOAD_COMMON(bearing_angular_velocity_rw);  // odometer --sbs
  LOAD_COMMON(estimate_frequency);  // odometer --sbs

  std::vector<double> imu_to_odom_leverarm;
  if (option_tools::safeGet(node, "imu_to_odom_leverarm", 
      &imu_to_odom_leverarm) && 
      imu_to_odom_leverarm.size() == 3) {
    for (size_t i = 0; i < 3; i++) {
      options.imu_to_odom_leverarm[i] = imu_to_odom_leverarm[i];
    }
  }
  else {
    LOG(INFO) << "Unable to load imu_to_odom_leverarm. Using default instead.";
  }

  std::vector<double> scaler;
  if (option_tools::safeGet(node, "scaler", 
      &scaler) && 
      scaler.size() == 2) {
    for (size_t i = 0; i < 2; i++) {
      options.scaler[i] = scaler[i];
    }
  }
  else {
    LOG(INFO) << "Unable to load scaler. Using default instead.";
  }
...
// template <>
// void loadOptions<PpprtkImuTcEstimatorOptions>(
//     YAML::Node& node, PpprtkImuTcEstimatorOptions& options)
// {
//   LOAD_COMMON(max_window_length);
// }
