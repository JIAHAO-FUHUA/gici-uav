  ...
  EstimatorBase(base_options)
{
  ...
  // spp_options.use_dual_frequency = true;   // test --sbs
...
bool PppEstimator::estimate()
{
  ...
  if (ppp_options_.use_ambiguity_resolution) {
    ...
    // get covariance of position + ambiguities
    Eigen::MatrixXd position_ambiguity_covariance = computeAndGetCovarianceAR(curState(),curAmbiguityState());
    CHECK_EQ(position_ambiguity_covariance.cols(), 3 + curAmbiguityState().ids.size());
    ...
    AmbiguityResolution::Result ret = ambiguity_resolution_->solvePpp(
      curState().id, curAmbiguityState().ids, position_ambiguity_covariance, curGnss());
