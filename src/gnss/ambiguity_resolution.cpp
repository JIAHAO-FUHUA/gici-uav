             ...
             )
{
  ...
  CHECK(ambiguity_covariance.cols()  == ambiguity_ids.size() + 3);  //--sbs
  ...
  for (size_t i = 0; i < ambiguity_size; i++) {
    for (size_t j = 0; j < ambiguity_size; j++) {
      ...
      ambiguity_covariance_(i, j) = ambiguity_covariance(3 + raw_i, 3 +  raw_j);
    }
    curAmbs()[i].std = sqrt(ambiguity_covariance_(i, i));
  }

  // covariance between ambiguity and position --sbs
  position_ambiguity_covariance_.resize(3, ambiguity_size);
  for (size_t i = 0; i < 3; i++) {
    for (size_t j = 0; j < ambiguity_size; j++) {
      const size_t raw_i = ambiguity_index_map.at(i);
      const size_t raw_j = ambiguity_index_map.at(j);
      position_ambiguity_covariance_(i, j) = ambiguity_covariance( raw_i, 3 +  raw_j);
    }
  }
  ...
  /// need float position, position_ambiguity_covariance, ambiguity_covariance, integer_ambiguity 
  //  to compute fixed position  --sbs
...
void AmbiguityResolution::formSatellitePairPpp()
{
  ...
  // 1) Single Frequency/ All Frequency ambiguity resolution



  // 2) Widelane combination 
  ...
  if (!use_rounding) {
    ...
    while (num_active >= min_num_fixation) {
      if (!(skip_full && num_active == float_ambiguities.size())) {
        if (sqrt(active_float_covariance(num_active - 1, num_active - 1)) < 0.25)   // (TODO:sbs) adjust the threshold
