                        ...
                        const Eigen::Vector3d position_cur)
{
  ...
  // Detect Pseudo-range/Doppler/Phase-range outlier/slip for Single-Frequency Measurements  --sbs
  rangeOutlierDetection(measurement_pre, measurement_cur, options);

  cycleSlipDetectionSf(measurement_pre, measurement_cur, options);
...
// Pseudo-range outlier detection
void rangeOutlierDetection(GnssMeasurement& measurement_pre, 
                          GnssMeasurement& measurement_cur,
                          const GnssCommonOptions& options)
{
  // Detect by Code-Doppler combination, from Wenjie Peng Wuhan Unniversity
  // Find valid code and doppler
  GnssMeasurementSDIndexPairs pairs = 
    gnss_common::formDopplerPseudorangeSDPair(measurement_pre, measurement_cur);
  ...
  // Form adjacent-epoch code-doppler combinations and detect
  double delta_time = measurement_cur.timestamp - measurement_pre.timestamp;
  for (size_t i = 0; i < pair_indexes.size(); i++) {
    for (size_t j = 0; j < pair_indexes[i].size(); j++) {
      GnssMeasurementIndex index_pre = pairs[pair_indexes[i][j]].rov;
      GnssMeasurementIndex index_cur = pairs[pair_indexes[i][j]].ref;
      Observation& observation_pre = measurement_pre.getObs(index_pre);
      Observation& observation_cur = measurement_cur.getObs(index_cur);

      double delta_Doppler = observation_cur.doppler - observation_pre.doppler;
      double delta_PD = fabs(observation_cur.pseudorange - observation_pre.pseudorange)
                    -fabs(0.5*(observation_cur.doppler + observation_pre.doppler)*delta_time);

      if (fabs(delta_Doppler) > options.delta_dop_thres && 
              fabs(delta_PD) > options.psr_dop_thres) {
        observation_cur.CEI = 1;
        observation_cur.DEI = 1;
#if LOG_CYCLE_SLIP
        LOG(INFO) << "Detected outlier in Doppler and Range at " 
          <<  measurement_cur.getSat(index_cur).prn 
          << ", delta_Doppler[m/s] = "<< delta_Doppler
          << ", delta_PD[m] = "<< delta_PD<<".";
#endif
      }
      else if (fabs(delta_PD > options.psr_dop_thres)) {
        observation_cur.CEI = 1;
#if LOG_CYCLE_SLIP
        LOG(INFO) << "Detected outlier in Range at " 
          <<  measurement_cur.getSat(index_cur).prn 
          << ", delta_PD[m] = "<< delta_PD<<".";
#endif      
      }
  ...
  // (TODO) Detect by C1-P1, P1-P2 combination, from Y.Gao Calgary

  // (TODO) Detect by SPP Fault Detectio and Exclusion (FDE)

}

// Cycle-slip Detection by Single Frequency measurements 
void cycleSlipDetectionSf(GnssMeasurement& measurement_pre, 
                          GnssMeasurement& measurement_cur,
                          const GnssCommonOptions& options)
{
  double epoch_factor = 1.0;
  double delta_time = measurement_cur.timestamp - measurement_pre.timestamp;
  if(delta_time <= 3.0) {
    epoch_factor = 1.0;
  }
  else if(delta_time <= 6.0){
    epoch_factor = 1.5;
  }
  else if(delta_time <= 12.0){
    epoch_factor = 2.0;
  }
  else{
    epoch_factor = 2.5;
  ...
  // Find valid code and phase
  // Detect by Code-Phase combination, from Wenjie Peng Wuhan Unniversity
  size_t i,j;
  GnssMeasurementSDIndexPairs pairs = 
  ...
  // Form adjacent-epoch code-phase combinations and detect
  for ( i = 0; i < pair_indexes.size(); i++) {
    for ( j = 0; j < pair_indexes[i].size(); j++) {
      GnssMeasurementIndex index_pre = pairs[pair_indexes[i][j]].rov;
      GnssMeasurementIndex index_cur = pairs[pair_indexes[i][j]].ref;
      Observation& observation_pre = measurement_pre.getObs(index_pre);
      Observation& observation_cur = measurement_cur.getObs(index_cur);

      double delta_PL = (observation_cur.pseudorange - observation_cur.phaserange)
                    -(observation_pre.pseudorange - observation_pre.phaserange);

      if ( observation_cur.CEI != 1 && observation_pre.CEI != 1 && 
            fabs(delta_PL) > options.psr_phr_thres * epoch_factor) {
        observation_cur.LLI = 1;
#if LOG_CYCLE_SLIP
        LOG(INFO) << "Detected Cycle-slip by Phase-Pseudo range combination at " 
          <<  measurement_cur.getSat(index_cur).prn 
          << ", delta_PL[m] = "<< delta_PL<<".";
#endif
      }
  ...
  // Detect by Phase-Doppler combination, from Wenjie Peng Wuhan Unniversity
  // Find valid code and doppler
  GnssMeasurementSDIndexPairs pairs_2 = gnss_common::formDopplerPhaserangeSDPair(measurement_pre, measurement_cur);

  // Find valid frequencies for each satellite
  std::vector<std::vector<int>> pair_indexes_2;
  last_prn  = "";
  for ( i = 0; i < pairs_2.size(); i++) {
    std::string prn = pairs_2[i].rov.prn;
    if (prn != last_prn) {
      std::vector<int> indexes; indexes.push_back(i);
      pair_indexes_2.push_back(indexes);
      last_prn = prn;
    }
    else {
      pair_indexes_2[pair_indexes_2.size() - 1].push_back(i);
    }
  }

  // Form adjacent-epoch phase-doppler combinations and detect
  for ( i = 0; i < pair_indexes_2.size(); i++) {
    for ( j = 0; j < pair_indexes_2[i].size(); j++) {
      GnssMeasurementIndex index_pre = pairs_2[pair_indexes_2[i][j]].rov;
      GnssMeasurementIndex index_cur = pairs_2[pair_indexes_2[i][j]].ref;
      Observation& observation_pre = measurement_pre.getObs(index_pre);
      Observation& observation_cur = measurement_cur.getObs(index_cur);

      double delta_LD = fabs(observation_cur.phaserange - observation_pre.phaserange)
                    -fabs(0.5*(observation_cur.doppler + observation_pre.doppler)*delta_time);

      if ( observation_cur.DEI != 1 && observation_pre.DEI != 1 
          && fabs(delta_LD) > options.phr_dop_thres * epoch_factor) {
        observation_cur.LLI = 1;
#if LOG_CYCLE_SLIP
        LOG(INFO) << "Detected Cycle-Slip by phase doppler combination at " 
          <<  measurement_cur.getSat(index_cur).prn 
          << ", delta_LD[m] = "<< delta_LD<<".";
#endif
      }
                        ...
                        const Eigen::Vector3d position_cur)
{
  ...
  for (size_t i = 0; i < pairs_pre.size(); i++) {
    ...
    double ddoppler = observation_rov.doppler - observation_ref.doppler; //--sbs
    
    Observation observation_sd;
    observation_sd.pseudorange = dpseudorange;
    observation_sd.phaserange = dphaserange;
    observation_sd.doppler = ddoppler; //--sbs
  ...
  // Detect Pseudo-range/Doppler/Phase-range outlier/slip for Single-Frequency Measurements  --sbs
  rangeOutlierDetection(measurement_sd_pre, measurement_sd_cur, options);

  cycleSlipDetectionSf(measurement_sd_pre, measurement_sd_cur, options);
  ...
  for (size_t i = 0; i < pairs_cur.size(); i++) {
    ...
    observation_rov.LLI |= observation_sd.LLI;
    observation_ref.LLI |= observation_sd.LLI;
    observation_rov.CEI |= observation_sd.CEI;
    observation_ref.CEI |= observation_sd.CEI;
    observation_rov.DEI |= observation_sd.DEI;
    observation_ref.DEI |= observation_sd.DEI;

  }

  // Detect by LLI again --sbs
  cycleSlipDetectionLLI(measurement_rov_pre, measurement_rov_cur);
  cycleSlipDetectionLLI(measurement_ref_pre, measurement_ref_cur);
                          ...
                          double threshold)
{
  ...
  // Form GF combinations and detect
  for (size_t i = 0; i < pair_indexes.size(); i++) {
    if (pair_indexes[i].size() < 2) continue; // freq >= 2 --sbs
...
void cycleSlipDetectionPosition(
                          ...
                          double threshold)
{
  // TODO: Detect by relative position from IMU/Odometer
