...
void PseudorangeErrorDD<Ns ...>::setInformation(const GnssErrorParameter& error_parameter)
{
  ...
  // test --sbs
  // double ele_weight = gnss_common::weight_elev(elevation_rov);
  // double ele_snr_weight = gnss_common::weight_elev_snr(elevation_rov, observation_rov_.SNR);
  // covariance_ = covariance_t(
  //   (square(ele_snr_weight)) * ratio );
  double gross_factor = 1.0;
  if(observation_rov_.CEI > 0.0)
  {
    gross_factor *= 5.0 ;
  }

  covariance_ = covariance_t(
    (square(factor(0)) + square(factor(1) / sin(elevation_rov))) * ratio * gross_factor * 4.0);
