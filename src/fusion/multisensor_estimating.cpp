  ...
  EstimatingBase(nodes, i_estimator), latest_imu_timestamp_(0.0)
{
  ...
  // load Odom base options
  YAML::Node odom_base_node = node["odom_estimator_base_options"];
  if (estimatorTypeContains(SensorType::GNSS, type_) && 
      odom_base_node.IsDefined()) {
    option_tools::loadOptions(odom_base_node, odom_base_options_);
    odom_base_options_.odom_parameters.sigma_v_c=odom_base_options_.forward_velocity_std;
    odom_base_options_.odom_parameters.sigma_g_c=odom_base_options_.bearing_angular_velocity_std;
    odom_base_options_.odom_parameters.l_S_Odom=odom_base_options_.imu_to_odom_leverarm;
    odom_base_options_.odom_parameters.scaler=odom_base_options_.scaler;
  ...
  else if (type_ == EstimatorType::RtkImuTc)
  {
    ...
    estimator_.reset(new RtkImuTcEstimator(
      ...
      gnss_loose_base_options_, imu_base_options_, odom_base_options_, base_options_, ambiguity_options_));
  }
  ...
  if (estimatorTypeContainsRtk(type_)) {
    rtk_estimator_.reset(new RtkEstimator(gnss_base_options_));
    // rtk_estimator_.base_options_.verbose_output = true;
  }
  // Initial values
...
bool MultiSensorEstimating::updateSolution()
{
  ...
  if (estimatorTypeContains(SensorType::GNSS, type_)) {
    if (std::shared_ptr<GnssEstimatorBase> gnss_estimator = 
      ...
      solution_.PDOP = gnss_estimator->getPdop(); //--sbs
    }
...
bool MultiSensorEstimating::processEstimator()
{
  ...
  if (solution_.coordinate == nullptr) 
  {
    ...
    // get coordinate zero from GNSS raw
      ...
      if(measurement.gnss->role == GnssRole::Rover)
      {
      ...
      // get coordinate from RTK fixed solution --sbs
      if (estimatorTypeContainsRtk(type_)) {
        if(measurement.gnss->role == GnssRole::Rover)
        {
          Solution sol; 
          // set coordinate
          sol.coordinate = std::make_shared<GeoCoordinate>(
            position_ecef, GeoType::ECEF);
          Eigen::Vector3d lla = sol.coordinate->convert(
            position_ecef, GeoType::ECEF, GeoType::LLA);
          rtk_estimator_->setCoordinate(sol.coordinate);

            LOG(INFO)<< "SPP solution for initial coordinate is done!"; //--sbs
        }
        if (!rtk_estimator_->addMeasurement(measurement)) {
          return false;
        }
        if ((rtk_estimator_->estimate()) ) {
          bool fixed_status = rtk_estimator_->isLastAmbiguityStatusFixed();
          if(fixed_status ){
            LOG(INFO)<<"RTK fixed solution is used to set coordinate!  Fixed status:"<<fixed_status ; //--sbs
            position_ecef = rtk_estimator_->getLastPositionEstimate();
            measurement.gnss->position = position_ecef;    
          }   
          else
          {
            LOG(INFO)<< "RTK fixed solution for initial coordinate is not available!"; //--sbs
            return false;
          }   
        }
        else 
        {
          LOG(INFO)<< "RTK for initial coordinate is not estimated!"; //--sbs
          return false;
        }
...
{
  SpinControl spin(SPIN_CONTROL_DURATION);
