  ...
  const GnssSolution& measurement)
{
  ...
  //--sbs
  if(has_any_velocity_measurement_ && gnss_solution_measurements_.size()<2 ||
    !has_any_velocity_measurement_ && gnss_solution_measurements_.size()<3)
  {
    LOG(INFO)<<"The dynamic window is too short, please adjust it and reboot !!";    
     return false;

  }


    LOG(INFO)<<" Motion initialization: full dynamic window !!";  //--sbs

  // Check acceleration or velocity
  Eigen::Vector3d initial_velocity;
  Eigen::Vector3d travel_position;
  bool acc_ensured = false;
  double max_acc = 0.0;
  for (size_t i = 1; i < gnss_solution_measurements_.size(); i++) {
    // if (has_any_velocity_measurement_) {

    //   // need fixed status --sbs
    //   // if (!(gnss_solution_measurements_[i].status == GnssSolutionStatus::Fixed && 
    //   //     gnss_solution_measurements_[i - 1].status == GnssSolutionStatus::Fixed)) {
    ...
    //   // Double check acceleration norm for dynamic alignment --sbs
    //   double acc_norm = fabs(getImuMeasurementNear(
    //       gnss_solution_measurements_[i - 1].timestamp).linear_acceleration.norm() - imu_base_options_.imu_parameters.g);
    //   if (acc_norm < 0.05) return  false;

    //   if (getImuMeasurementNear(gnss_solution_measurements_[i].timestamp).angular_velocity.norm()  * R2D  > 5 ||
    //   getImuMeasurementNear(gnss_solution_measurements_[i].timestamp).angular_velocity.norm() < 0.01) return false;
    //   if (getImuMeasurementNear(gnss_solution_measurements_[i-1].timestamp).angular_velocity.norm()* R2D  > 5 ||
    //   getImuMeasurementNear(gnss_solution_measurements_[i-1].timestamp).angular_velocity.norm() < 0.01 ) return false;
    // }
    // else if
    if (i > 1) {
      // need fixed status --sbs
      if (!(gnss_solution_measurements_[i].status == GnssSolutionStatus::Fixed && 
          gnss_solution_measurements_[i - 1].status == GnssSolutionStatus::Fixed &&
          gnss_solution_measurements_[i - 2].status == GnssSolutionStatus::Fixed)) {
          gnss_solution_measurements_.clear(); 
          dynamic_window_full_ = false; break;
      }
      ...
      Eigen::Vector3d cur_position = coordinate_->convert(
        cur.position, GeoType::ECEF, GeoType::ENU);
      Eigen::Vector3d last_last_position = coordinate_->convert(
        last_last.position, GeoType::ECEF, GeoType::ENU);
      double hor_distance = (cur_position - last_last_position).head<2>().norm();
      if (imu_base_options_.car_motion) {
        const double angular_velocity_norm = getImuMeasurementNear(
          gnss_solution_measurements_[i - 1].timestamp).angular_velocity.norm();
        if (hor_distance < options_.min_acceleration || 
            angular_velocity_norm > 
            imu_base_options_.car_motion_max_anguler_velocity) {
          gnss_solution_measurements_.clear(); 
          LOG(INFO)<<"Motion initialization: Passing straight"<< hor_distance <<" < "
                << options_.min_acceleration <<" m !!";  //--sbs
      ...
      LOG(INFO)<<"Motion initialization: Passing straight "<< options_.min_acceleration <<"m !!";  //--sbs


      // initial_velocity = last_velocity;
      initial_velocity = (last_velocity+velocity)/2.0; //--sbs
      ...
      double acc_norm = fabs(getImuMeasurementNear(
          gnss_solution_measurements_[i - 1].timestamp).linear_acceleration.norm() - imu_base_options_.imu_parameters.g);
      // if (acc_norm < options_.min_acceleration) return false;  

      // if (getImuMeasurementNear(gnss_solution_measurements_[i].timestamp).angular_velocity.norm()  * R2D  > 5 ||
      // getImuMeasurementNear(gnss_solution_measurements_[i].timestamp).angular_velocity.norm()   < 0.01) return false;
      // if (getImuMeasurementNear(gnss_solution_measurements_[i-1].timestamp).angular_velocity.norm()* R2D  > 5 ||
      // getImuMeasurementNear(gnss_solution_measurements_[i-1].timestamp).angular_velocity.norm()  < 0.01) return false;
      // if (getImuMeasurementNear(gnss_solution_measurements_[i-2].timestamp).angular_velocity.norm()* R2D  > 5 ||
      // getImuMeasurementNear(gnss_solution_measurements_[i-2].timestamp).angular_velocity.norm()   < 0.01) return false;
    }
...
void GnssImuInitializer::slowMotionInitialization()
{
  ...
  if (timestamp_start >= imu_measurements_.front().timestamp) {
    ...
    if (initPoseAndBiases(imu_measurements_, 
      ...
      // speed_and_bias_0_(5) = 0.061; // init gyro bias, rad/s,  test--sbs
  ...
  const Eigen::Quaterniond& q_WS_0, const SpeedAndBias& speed_and_bias_0)
{
  ...
  for (size_t i = 0; i < gnss_solution_measurements_.size(); i++)
  {
    ...
    // add covariance for fixed --sbs
    else
    {
      gnss.covariance.topLeftCorner(3, 3) = Eigen::Matrix3d::Identity() * 1.0e-2;
    }
    ...
    if (isFirstEpoch()) {
      ...
      // test --sbs
      double speed_std = speed_and_bias.head<3>().norm() * 10.0;
      if (speed_std < 1.0) speed_std = 1.0;
      ...
      // test --sbs
      if (imu_base_options_.car_motion) {
        double std_yaw = sqrt(square(imu_base_options_.body_to_imu_rotation_std * D2R * 2.0) + 
          square(0.1 / speed_and_bias.head<2>().norm()));
        addPoseResidualBlock(states_[index], T_WS, 0.1, std_yaw);
      }      
    }
