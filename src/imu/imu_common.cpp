                       ...
                       const double timestamp_end)
{
  ...
  for (size_t i = 0; i < imu_measurements.size(); ++i)
  {
    ...
    // only use X/Y acc norm + gyro norm to check static --sbs
    // double hor_acc_norm = fabs(imu_measurements[i].linear_acceleration.head<2>().norm());
    // if (hor_acc_norm > 0.05) continue;
    if (fabs(imu_measurements[i].angular_velocity.norm()) > 0.05) continue;
