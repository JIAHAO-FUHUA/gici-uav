...
struct GnssEstimatorBaseOptions {
  ...
  // Maximum number of available pseudorange measurements --sbs
  int max_pesudorange_size = 20;

  int max_satellite_size = 20;
};
...
public:
  ...
  // // Get solution status --sbs
  // inline GnssSolutionStatus getSolutionStatus() { return latestState().status; }

  // Get number of satellite
  inline int getNumberSatellite() { return num_satellites_; }

  // Get PDOP
  inline double getPdop() { return pdop_; }
...
protected: 
  ...
  // compute pdop --sbs
  inline void updatePdop(
    const GnssMeasurement& measurement_rov, const GnssMeasurementDDIndexPairs& indexes) {
    pdop_ = gnss_common::computeDops(
      measurement_rov, indexes, gnss_base_options_.common).y();
  }
