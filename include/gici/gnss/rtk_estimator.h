...
public:
  ...
  RtkEstimator(const GnssEstimatorBaseOptions& gnss_base_options); //--sbs
  ...
  // Get position estimate in ECEF --sbs
  inline Eigen::Vector3d getLastPositionEstimate() {
    return getPositionEstimate(lastState());
  }
  // Check if last ambiguity status fixed --sbs 
  inline bool isLastAmbiguityStatusFixed() {
    return (lastState().status ==GnssSolutionStatus::Fixed && num_satellites_ >= 12 && pdop_ < 2.0);
  }
  // // Check if last ambiguity status fixed --sbs 
  // inline int lastSatNum() {
  //   return (lastState().num_satellites);
  // }
