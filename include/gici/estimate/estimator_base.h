...
protected:
  ...
  // Compute and get position+ambiguity covariance at a given state --sbs
  Eigen::MatrixXd computeAndGetCovarianceAR
      (const State& state,
...
protected:
  ...
  // For time cost --sbs
  std::chrono::system_clock::time_point tic_start = std::chrono::system_clock::now();
  std::chrono::system_clock::time_point tic_end = std::chrono::system_clock::now();
