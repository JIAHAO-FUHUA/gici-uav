...
#include "gici/imu/odom_types.h"  //--sbs
namespace gici {
...
public:
  ...
  inline static SensorType sensorType(IdType type) {
    ...
    if (type==IdType::oPose || type == IdType::oScaler) return SensorType::Odometer; //--sbs
...
inline BackendId createOdomPoseId(int32_t bundle_id)
{
  CHECK_GE(bundle_id, 0);
  return BackendId(
    BackendId::setBits(BackendId::adjustBundleId(
      bundle_id, IdType::oPose), BITS_BUNDLEID) |
    BackendId::setBits(IdType::oPose, BITS_IDTYPE));
}
...
inline BackendId createOdomScalerId(int32_t bundle_id)
{
  CHECK_GE(bundle_id, 0);
  return BackendId(
    BackendId::setBits(BackendId::adjustBundleId(
      bundle_id, IdType::oScaler), BITS_BUNDLEID) |
    BackendId::setBits(IdType::oScaler, BITS_IDTYPE));
}
...
// --sbs
inline BackendId createOdometerScalerId(int32_t bundle_id)
{
  CHECK_GE(bundle_id, 0);
  return BackendId(
    BackendId::setBits(BackendId::adjustBundleId(
...
inline BackendId changeIdType(BackendId id, IdType type, size_t cam_index = 0)
{
  ...
  // // odometer --sbs
  // else if (type == IdType::oScaler){
  //   out = BackendId::resetBits(out, id.type(), BITS_BUNDLEID);
  // }
...
public:
  ...
  EstimatorDataCluster(const OdomMeasurement& data, const OdomRole& role, const std::string& tag) :  //--sbs
    odometer(std::make_shared<OdomMeasurement>(data)),
    odom_role(role), tag(tag), timestamp(data.timestamp) {}
  ...
  std::shared_ptr<OdomMeasurement> odometer;  //--sbs
