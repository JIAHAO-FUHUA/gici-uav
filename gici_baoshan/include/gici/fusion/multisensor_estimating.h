...
private:
  ...
  // Estimator type with RTK checker --sbs
  inline bool estimatorTypeContainsRtk(EstimatorType estimator_type) {
  ...
  inline bool estimatorTypeContains(
    ...
    if (sensor_type == SensorType::GNSS) {
      return (estimator_type == EstimatorType::Spp || 
              ...
              estimator_type == EstimatorType::PpprtkImuTc ||     //--sbs
              estimator_type == EstimatorType::GnssImuCameraSrr || 
  ...
  inline bool estimatorDataIllegal(const EstimatorDataCluster& data) {
    ...
    if (data.odometer) num_data_type++; //--sbs
    if (num_data_type > 1) {
  ...
  // Check if the data will be used for time propagation (only support Odom)
  inline bool estimatorDataIsOdom(const EstimatorDataCluster& data) {
    return (data.odometer != nullptr);
  }
...
protected:
  ...
  std::unique_ptr<RtkEstimator> rtk_estimator_; //--sbs
  ...
  OdomEstimatorBaseOptions odom_base_options_;  //--sbs
  ...
  // PpprtkImuTcEstimatorOptions ppprtk_imu_tc_options_;
