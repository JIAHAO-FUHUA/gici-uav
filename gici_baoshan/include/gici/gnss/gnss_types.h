...
#include "gici/gnss/atmnetwork.h" //--sbs
namespace gici {
...
struct Observation {
  ...
  uint8_t CEI; // Code Error Indicator --sbs
  uint8_t DEI; // Doppler Error Indicator --sbs
};
...
struct Satellite {
  ...
  double ionosphere_sd; // satellite-differenced ionosphere --sbs
  double ionosphere_sd_std; // satellite-differenced ionosphere std
  std::string ionosphere_base_prn_G; // satellite-differenced ionosphere's base satellite gps
  std::string ionosphere_base_prn_R; // satellite-differenced ionosphere's base satellite glonass
  std::string ionosphere_base_prn_E; // satellite-differenced ionosphere's base satellite galileo
  std::string ionosphere_base_prn_C; // satellite-differenced ionosphere's base satellite bds2
  std::string ionosphere_base_prn_C3; // satellite-differenced ionosphere's base satellite bds3
...
// using ionnetworks = std::deque<std::pair<double, ionnetwork>>; //--sbs
// using ztdnetworks = std::deque<std::pair<double, ztdnetwork>>; //--sbs
...
struct GnssMeasurement {
  ...
  double troposphere_wet_std;  // Wet ZTD std from augmentation

  ionnetwork ionospheric_network; //(TODO:--sbs)
  ztdnetwork tropospheric_network;
...
struct GnssCommonOptions {
  ...
  Eigen::Vector2d min_SNR = Eigen::Vector2d(0.0, 0.0);
  ...
  // Threshold for pseudorange/doppler combination to detect pseudorange error (m) --sbs
  double psr_dop_thres = 10.0 * 2;
  double delta_dop_thres = 10.0 * 2;
  double phr_dop_thres = 5.0 * 2;
  double psr_phr_thres = 15.0 * 2;
...
public:
  ...
  std::map<std::string, BackendId> map_prn_ids;  //--sbs

  inline void clear() {
    ids.clear(); timestamp = 0.0; map_prn_ids.clear();  
  }
};
