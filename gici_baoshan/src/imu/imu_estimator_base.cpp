  ...
  const double timestamp, Transformation& T_WS)
{
  ...
  // the i = states_.size()-1 is an empty state added in estimate()--sbs
  int end_index =  states_.size() - 2;
  for (int i = states_.size() - 2; i >= 0; i--) {   
...
// Check if the state is valid for non-holonomic or Odometer constraint --sbs
bool ImuEstimatorBase::checkHmcValid(const State& state)
{
  BackendId pose_id = state.id_in_graph;
  ...
  if (angular_velocity_norm * R2D > 
    imu_base_options_.car_motion_max_anguler_velocity) return false;
...
void ImuEstimatorBase::eraseImuResidualBlock(const State& last_state, State& cur_state)
{
  ...
  // --sbs
  if(!graph_->removeResidualBlock(cur_state.imu_residual_to_lhs))
  { 
