...
//  --sbs
bool compareSNR(const std::pair<std::string,Satellite>& s1, const std::pair<std::string,Satellite>& s2) {
  return s1.second.observations.begin()->second.SNR > s2.second.observations.begin()->second.SNR;
}

double getNthLargestSNR(const Satellites& satellites, int n) {
  if(n > satellites.size()) return 0.0;
  std::vector<std::pair<std::string,Satellite>> sorted_satellites(satellites.begin(),satellites.end());
  std::sort(sorted_satellites.begin(), sorted_satellites.end(), compareSNR);
  return sorted_satellites[n-1].second.observations.begin()->second.SNR;
}


void deleteSatelliteToSize(GnssMeasurement& measurement, int num)
{
  // delete satellite if the number of satellites is larger than num
  if(measurement.satellites.size() <= num) return;

  double SNR_thres = getNthLargestSNR(measurement.satellites, num);

  if(SNR_thres <= 0.0) return;
  LOG(INFO)<<num<<"th Largest SNR = "<<SNR_thres;

  // for (auto& sat : measurement.satellites) {
  int sat_num_all = measurement.satellites.size();
  int sat_num_erase = 0;
  for(auto sat = measurement.satellites.begin(); sat != measurement.satellites.end(); ) {
    if(sat->second.observations.begin()->second.SNR < SNR_thres) {
      LOG(INFO)<<"erase satellite "<<sat->first;
      sat_num_erase++;
      sat = measurement.satellites.erase(sat);
    }
    else {
      ++sat;
    }
  }
  LOG(INFO)<<"erase satellite done, "<<sat_num_all<<" - "<<sat_num_erase << "= "<<measurement.satellites.size()<<" satellites remained";
...
// Form single difference doppler + pseudorange pair
GnssMeasurementSDIndexPairs formDopplerPseudorangeSDPair(
                            const GnssMeasurement& measurement_rov, 
                            const GnssMeasurement& measurement_ref,
                            const GnssCommonOptions& options)
{
  ...
  for (auto& sat : measurement_rov.satellites) 
  {
    ...
    for (auto obs : satellite.observations) {
      GnssMeasurementIndex index_rov(satellite.prn, obs.first);
      if (checkObservationValid(measurement_rov, index_rov,
          ObservationType::Doppler, options) && 
          checkObservationValid(measurement_rov, index_rov,
          ObservationType::Pseudorange, options)) {
        indexes_1.push_back(index_rov);
      }
  ...
  for (auto index : indexes_1) 
  {
    ...
    for (auto& sat : measurement_ref.satellites) {
      ...
      for (auto& obs_2 : satellite_2.observations) {
        ...
        if (!checkObservationValid(measurement_ref, 
            GnssMeasurementIndex(satellite.prn, index.code_type), 
            ObservationType::Doppler)) continue;
...
// (TODO: debug )Form single difference doppler + phaserange pair   --sbs
GnssMeasurementSDIndexPairs formDopplerPhaserangeSDPair(
                            const GnssMeasurement& measurement_rov, 
                            const GnssMeasurement& measurement_ref,
                            const GnssCommonOptions& options)
{
...
double weight_elev(double elevation) {
  if(elevation <= 30.0) return 1.0 / (2.0 * sin(elevation));
  else return 1.0;
}

// double weight_snr(double snr) {
// //  double T1 = 40.0, F1 = 20.0, A1 = 30.0, a1 = 30.0;
//   // double s1 = 50.0, s0 = 20.0, A = 50.0, a= 30.0;
//   double s1 = 50.0, s0 = 10.0, A = 30.0, a= 20.0;
// //  double T1 = 50.0, F1 = 10.0, A1 = 30.0, a1 = 30.0;
// //  double T1=50.0, F1=25.0, A1=34.0, a1=43.0;
//   double weight = 0.0;
//   if(snr >= s1) {
//     weight = 1.0;
//   } else {
//     // weight = pow(10, -(snr - T) / a) *
//     //          (1.0 + (A / (pow(10, -(F - T) / a)) - 1.0) * (snr - T) / (F - T));

//     weight = 1.0 / (2.0 * sin(elevation))*
//              pow(10, -(snr-s1)/a) * 
//              (1.0 + (A / (pow(10, - (s0 - s1) / a)) - 1.0) * ((snr-s1)/(s0-s1)));
//   }
//   //LOGW << "weig snr " << snr <<" "<<weight;
//   return weight;
// }


double weight_elev_snr(double elevation, double snr){
//  double T1 = 40.0, F1 = 20.0, A1 = 30.0, a1 = 30.0;
  // double s1 = 50.0, s0 = 20.0, A = 50.0, a= 30.0;
  double s1 = 50.0, s0 = 10.0, A = 30.0, a= 20.0;
//  double T1 = 50.0, F1 = 10.0, A1 = 30.0, a1 = 30.0;
//  double T1=50.0, F1=25.0, A1=34.0, a1=43.0;
  ...
  if(snr >= s1) {
    ...
    weight = 1.0 / (sin(elevation) * sin(elevation))*
             pow(10, -(snr-s1)/a) * 
             (1.0 + (A / (pow(10, - (s0 - s1) / a)) - 1.0) * ((snr-s1)/(s0-s1)));
  }
  //LOGW << "weig snr " << snr <<" "<<weight;
