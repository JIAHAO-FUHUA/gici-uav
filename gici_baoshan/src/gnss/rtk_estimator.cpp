...
// The second constructor for initialization --sbs
RtkEstimator::RtkEstimator(const GnssEstimatorBaseOptions& gnss_base_options) :
  rtk_options_(RtkEstimatorOptions()), ambiguity_options_(AmbiguityResolutionOptions()),
  ...
  EstimatorBase(EstimatorBaseOptions())
{
  ...
  spp_estimator_.reset(new SppEstimator(gnss_base_options));
    // Ambiguity resolution
  ambiguity_resolution_.reset(new AmbiguityResolution(AmbiguityResolutionOptions(), graph_));
    ...
    const GnssMeasurement& measurement_ref)
{
  ...
  updatePdop(curGnssRov(), code_index_pairs);
  return true;
}
...
// #ifdef DEBUG
    // LOG(INFO)<<"Ambiguity covariance: "<<std::endl<<ambiguity_covariance<<std::endl;
// #endif
    // solve
  ...
  if (base_options_.verbose_output) {
    ...
    LOG(INFO) << estimatorTypeToString(type_) << ": " 
      ...
      << ", PDOP: " << std::setprecision(1) << std::fixed << pdop_
...
Eigen::Vector3d RtkEstimator::getPositionEstimate(const State& state)
{
