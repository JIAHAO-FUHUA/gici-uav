  ...
  AmbiguityState& state)
{
  ...
  for (auto& sat : measurement.satellites) 
  {
    ...
    for (auto obs : satellite.observations) {
      ...
      // }
      // add first ambiguity residual block to avoid rank deficiency --sbs 
      if (!has_last) {
  ...
  bool reject_one)
{
  ...
  // check if residual size is smaller than certain amount --sbs
  if (residuals.size() - outlier_indexes.size() > gnss_base_options_.max_pesudorange_size) {
    int size_to_remove = residuals.size() - gnss_base_options_.max_pesudorange_size;
    std::vector<int> tmp_indices(residuals.size());
    std::iota(tmp_indices.begin(), tmp_indices.end(), 0);
    std::partial_sort(tmp_indices.begin(), tmp_indices.begin()+size_to_remove, tmp_indices.end(),
              [&](int a, int b){return residuals[a] > residuals[b];});
    outlier_indexes.clear();
    outlier_indexes = std::vector<int>(tmp_indices.begin(), tmp_indices.begin()+size_to_remove);

    LOG(INFO)<< "Rejecting " << size_to_remove << " pseudorange outliers, remaining " << gnss_base_options_.max_pesudorange_size;
  }
